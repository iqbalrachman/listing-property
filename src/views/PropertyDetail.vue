      ...
      <div class="layout">
        ...
        <div class="info">
          ...
          <div class="specs mono">
            <div v-if="property.land_area"><span>LT</span>{{ property.land_area }} m²</div>
            <div v-if="property.building_area"><span>LB</span>{{ property.building_area }} m²</div>
            <div v-if="property.bedroom"><span>KT</span>{{ property.bedroom }}</div>
            <div v-if="property.bathroom"><span>KM</span>{{ property.bathroom }}</div>
            <div v-if="property.certificate"><span>Sertifikat</span>{{ property.certificate }}</div>
          </div>

          <a v-if="waLink" :href="waLink" target="_blank" rel="noopener" class="btn btn-brass wa-btn">
            Tanya via WhatsApp
          </a>
...
.location { color: var(--text-muted); margin: 0 0 12px; }
.price {
  font-size: 24px;
  font-weight: 600;
...
.description p { line-height: 1.7; color: #2c3554; white-space: pre-line; }
.video-frame {
  position: relative;
  aspect-ratio: 16 / 9;
  border-radius: var(--radius);
